/* 
              ...
              Este código é copiado do professor e tem a função de demonstrar o uso do Joystick Arcade para fliperama com o Arduíno que, junto com um LCD I2C, consegue-se
       fazer um jogo. O jogo feito neste código é o "Pong", que já tinha sido feito, mas controlado com um potenciômetro. 
...
// Arduino Pins
const int cima_b     = 3;             // Tecla Joystick cima
const int baixo_b    = 4;             // Tecla Joystick baixo
const int compPoint1 = 6;             // LED1 pin
const int compPoint2 = 11;            // LED2 pin
const int compPoint3 = 12;            // LED3 pin
...
int paddleNumber = 1;                 // Posição do Paddle (inicia em 1)

boolean cima_f = 0, baixo_f = 0;      // Flags para as teclas do Joystick
...
void setup() 
{
  ...
  // set buttons pin as input
  pinMode(button, INPUT);
  pinMode(cima_b,  INPUT_PULLUP);
  pinMode(baixo_b, INPUT_PULLUP);
...
// Função para leitura do Joystick (para cima/para baixo)
void readJoystick()
{
  if(!digitalRead(cima_b)) paddleNumber += 1;
/*
  if(digitalRead(cima_b) && cima_f)
  {
    cima_f = 0x00;
    paddleNumber += 1;
...
*/
  ...
  if(!digitalRead(baixo_b)) paddleNumber -= 1;
/*
  if(digitalRead(baixo_b) && baixo_f)
  {
    baixo_f = 0x00;
    paddleNumber -= 1;  
    digitalWrite(13, !digitalRead(13));
...
/*    paddlePos = constrain(map(potReading, 0, 950, 1, 14), 1, 14);    */
    ...
    paddlePos = constrain(paddleNumber, 1, 14);
